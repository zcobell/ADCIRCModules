...
float kdtree2_result_vector::replace_maxpri_elt_return_new_maxpri(
    kdtree2_result& e) {
...
kdtree2_node* kdtree2::build_tree_for_range(int l, int u,
                                            kdtree2_node* parent) {
  ...
  if ((u - l) <= bucketsize) {
    ...
    if (node->right == NULL) {
      ...
      for (int i = 0; i < dim; i++) {
        node->box[i].upper =
            max(node->left->box[i].upper, node->right->box[i].upper);

        node->box[i].lower =
            min(node->left->box[i].lower, node->right->box[i].lower);
      }
    }
  }
  return (node);
...
void kdtree2::n_nearest_brute_force(vector<float>& qv, int nn,
...
void kdtree2::n_nearest(vector<float>& qv, int nn,
                        kdtree2_result_vector& result) {
...
void kdtree2::r_nearest(vector<float>& qv, float r2,
                        kdtree2_result_vector& result) {
