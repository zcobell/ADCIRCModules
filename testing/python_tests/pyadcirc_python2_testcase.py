#!/usr/bin/env python2
import adcircmodules
import math

print "Reading a mesh"
m = adcircmodules.Mesh("../testing/test_files/ms-riv.grd")
m.read()
...
f = adcircmodules.NodalAttributes("../testing/test_files/ms-riv.13",m)
f.read()
...
o = adcircmodules.ReadOutput("../testing/test_files/maxele.63")
o.open();
o.read();
o.close()
...
onc = adcircmodules.ReadOutput("../testing/test_files/maxele.63.nc")
onc.open()
onc.read()
onc.close();
print "   Output value for reacord 1 at node 42: ",onc.data(0).z(41)
print "Reading ascii timeseries scalar file"
asc = adcircmodules.ReadOutput("../testing/test_files/sparse_fort.63")
asc.open();
asc.read();
asc.read();
asc.read();
...
ncv=adcircmodules.ReadOutput("../testing/test_files/fort.64.nc")
ncv.open();
ncv.read();
ncv.read();
...
print "       Direction is: ",ncv.data(2).direction(1220,adcircmodules.Radians), "radians or ",ncv.data(2).direction(1220,adcircmodules.Degrees)," degrees"
print "Success reading netcdf vector timeseries file"
ncv.close()
print "Reading harmonics elevation file"
he = adcircmodules.HarmonicsOutput("../testing/test_files/fort.53");
he.read();
...
hv = adcircmodules.HarmonicsOutput("../testing/test_files/fort.54");
hv.read();
