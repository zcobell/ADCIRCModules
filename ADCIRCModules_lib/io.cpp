...
int IO::splitString(std::string &data, std::vector<std::string> &result)
{
  ...
  boost::algorithm::split(result,data,boost::is_any_of(" "),boost::token_compress_on);
  return 0;
}


int IO::splitStringNodeFormat(std::string &data, int &id, double &x, double &y, double &z)
{
...
int IO::splitStringElemFormat(std::string &data, int &id, int &n1, int &n2, int &n3)
{
