...
class FaceNode {
 public:
  explicit FaceNode(Adcirc::Geometry::Node *node)
...
class Face {
 public:
  ...
  explicit Face(Adcirc::Geometry::Node *n1 = nullptr,
  ...
  Face(std::pair<Adcirc::Geometry::FaceNode, Adcirc::Geometry::FaceNode> n,
      ...
      : m_nodes(std::move(n)), m_elements(std::move(e)) {}
  ...
  void setNodes(const std::pair<Adcirc::Geometry::Node *,
    ...
    m_nodes.first = FaceNode(nodes.first);
    m_nodes.second = FaceNode(nodes.second);
  }
