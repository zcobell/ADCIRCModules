...
double Constants::radiusEarth(double latitude) {
  ...
  return std::sqrt(
      (std::pow(Constants::equatorialRadius(), 4.0) * std::cos(l) *
           std::cos(l) +
       std::pow(Constants::polarRadius(), 4.0) * std::sin(l) * std::sin(l)) /
      (std::pow(Constants::equatorialRadius(), 2.0) * std::cos(l) *
           std::cos(l) +
 ...
 * @return distance between two points
 */
double Constants::distance(const Point &p1, const Point &p2, bool geodesic) {
  if (geodesic) {
    return Constants::geodesic_distance(p1.x(), p1.y(), p2.x(), p2.y());
  } else {
    return Constants::cartesian_distance(p1.x(), p1.y(), p2.x(), p2.y());
  }
}
...
double Constants::distance(const Point &p1, double x2, double y2,
                           bool geodesic) {
  if (geodesic) {
    return Constants::geodesic_distance(p1.x(), p1.y(), x2, y2);
  } else {
    return Constants::cartesian_distance(p1.x(), p1.y(), x2, y2);
  }
}
